...
export const colorChosenStation = (svgCanvas, stationName) => {
  svgCanvas.selectAll('.station-point')
    .filter(d => d.properties.Name === stationName)
    .attr('fill', d => {
      const lineInfo = linesWithColors.find(
        line => line.Ligne === d.properties.Line
      );
      return lineInfo ? lineInfo.color : 'white';
    });
};
